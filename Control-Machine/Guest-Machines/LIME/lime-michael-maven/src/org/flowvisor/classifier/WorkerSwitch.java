...
//				System.out.println("MURAD: WorkerSwitch, " + this.getName() + " message sent fine: "+msg);
			...
			System.out.println("NOFEL WorkerSwitch sendMsg: Printing Trace: ");
	 ...
	 * Modify flowmod with action output to empty port. This occurs in the CLONE
	 * switch object, not the original switch.
	 * 
	 * Should create a flowmod in the clone of a the original switch that add a vlan tag
	 * and sends the packet out the ghost port. For now, the vlan tag is the port on the 
	 * original switch on which to output the packet. 
	 * 
	 * A flowmod must also be written to the original switch to strip the vlan tag and
	 * output on the port equal to the vlan tag #. Also need to change flow mods in the original
	 * that output on the port with the vlan tag # to instead add a vlan tag and output
	 * on the ghost port.
	 * 
	 * Can be used to create original cloned flows or flows that need to be made after each host
	 * is migrated
	 * 
	 * In future, we will need to have a
	 * translation table to map incoming vlan tags to the original rule (e.g. each rule 
	 * in the original switch will have a vlan tag associated and will then have a rule
	 * written in the original switch that strips the tag and performs the actions of the
	 * original rule)
...
//		cloneActiveSwitch.flowTable.copyFromAnotherLimeFlowTable(originalActiveSwitch.flowTable);
		...
		ArrayList<FVFlowMod> sendingSwitchMods = new ArrayList<FVFlowMod>();
		ArrayList<FVFlowMod> receivingSwitchMods = new ArrayList<FVFlowMod>();
		for (FVFlowMod flowMod : flowTable) {
...
//			System.out.println("\nCurrent flowmod: "+flowMod+"\n");
	...
	public static boolean hasVlanAction(FVFlowMod flowMod) {
		List<OFAction> actions = flowMod.getActions();
		boolean containsVlan = false;
	...
	public static boolean hasOutputPortWithoutVlan(FVFlowMod flowMod, Short port) {
		List<OFAction> actions = flowMod.getActions();
	...
	private static FVFlowMod createVlanHandlerMod(FVFlowMod originalMod, short vlanNumber, WorkerSwitch handlerSwitch) {
...
//		handlerSwitch.handleFlowModAndSend(newMod, true);
	 ...
	 * @return A map of the vlan tag number to the flowmod
	 */
	public static HashMap<Short, FVFlowMod> createVlanSenderMod(FVFlowMod flowMod, WorkerSwitch senderSwitch){
		//create new mod, assuming the clone function works
		// System.out.println("NOFEL createVlanSenderMod Line 1");
		FVFlowMod clonedMod = (FVFlowMod) flowMod.clone();
		// System.out.println("NOFEL createVlanSenderMod Line 2");
		System.out.println("\nCurrent flowmod: "+flowMod+"\n");
		short vlanNumber = -1;
		...
		for(int i = 0; i<clonedMod.getActions().size(); i++ ){
			// System.out.println("NOFEL createVlanSenderMod Line 3");
			OFAction action = clonedMod.getActions().get(i);
			// System.out.println("NOFEL createVlanSenderMod Line 4");
			if(action instanceof OFActionOutput){
				// System.out.println("NOFEL createVlanSenderMod Line 5");
				if(senderSwitch.getActivePorts().containsKey(((OFActionOutput) action).getPort())){
					// System.out.println("NOFEL createVlanSenderMod Line 6");
...
//					}
				...
				// System.out.println("NOFEL createVlanSenderMod Line 7");
			}
			// System.out.println("NOFEL createVlanSenderMod Line 8");
		}
		// System.out.println("NOFEL createVlanSenderMod Line 9");
//		System.out.println("Flow mod being sent: "+flowMod);
		//write flowmod to the clone switch
//		senderSwitch.handleFlowModAndSend(clonedMod, true);
		HashMap<Short, FVFlowMod> map = new HashMap<Short, FVFlowMod>();
		map.put(vlanNumber, clonedMod);
		// System.out.println("NOFEL createVlanSenderMod Line 10");
		return map;
	}
...
//		}
	...
	public String printWorkerSwitch() {
		System.out.println("PRINT ADDED BY SAIM SALMAN. \n");
		System.out.println("Loop: " + loop.toString());
		System.out.println("Sock: " + sock.toString());
		System.out.println("SwitchName: " + switchName);
		System.out.println("DoneID: " + doneID);
		System.out.println("Connected Host Ports: " + connectedHostPorts.toString());
		System.out.println("Connected Host Ports Counter: " + connectedHostPortsCounter);		
		System.out.println("isActive: " + isActive);
		System.out.println("duplicateSwitch: " + duplicateSwitch.toString());
		System.out.println("activePorts: " + activePorts.toString());
		System.out.println("flowTable: " + flowTable.dump());
		System.out.println("msgStream: " + msgStream.toString());
		System.out.println("switchInfo: " + switchInfo.toString());
		System.out.println("originalSwitchMap: " + originalSwitchMap.toString());
		System.out.println("xidTranslator: " + xidTranslator.toString());
		System.out.println("cookieTranslator: " + cookieTranslator.toString());
		System.out.println("missSendLength: " + missSendLength);
		System.out.println("switchFlowMap: " + switchFlowMap.toString());
		System.out.println("shutdown: " + shutdown);
		System.out.println("keepAlive: " + keepAlive.toString());
		System.out.println("stats: " + stats.toString());
		System.out.println("flowDB: " + flowDB.toString());
		System.out.println("wantStatsDescHack: " + wantStatsDescHack);
		System.out.println("floodPermsSlice: " + floodPermsSlice.toString());
		System.out.println("flowTracking: " + flowTracking.toString());
		System.out.println("flowTableList: " + flowTableList.toString());
		System.out.println("registeredForFlowTable: " + registeredForFlowTable.toString());
		System.out.println("fmlimits: " + fmlimits.toString());
		System.out.println("currfmlimits: " + currfmlimits.toString());
		System.out.println("statsWindowOpen: " + statsWindowOpen);
		System.out.println("flowStats: " + flowStats.toString());
		System.out.println("actualStats: " + actualStats.toString());
		System.out.println("toDeleteSlices: " + toDeleteSlices.toString());
		System.out.println("PRINT ADDED BY SAIM SALMAN. \n");
		return "\nSwitch Completely Printed";
	}
